...
#include "GarAnimNotifyState_EarlyBlendOut.generated.h"

UCLASS(DisplayName = "Gar Early Blend Out Animation Notify State")
class GAR_API UGarAnimNotifyState_EarlyBlendOut : public UAnimNotifyState
{
	GENERATED_BODY()
...
protected:
	...
	FGameplayTag LocomotionModeEquals{GarLocomotionModeTags::InAir};

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Settings", Meta = (EditCondition = "bCheckRotationMode"))
	FGameplayTag RotationModeEquals{GarRotationModeTags::Aiming};

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Settings", Meta = (EditCondition = "bCheckStance"))
	FGameplayTag StanceEquals{GarStanceTags::Crouching};

public:
	UGarAnimNotifyState_EarlyBlendOut();
