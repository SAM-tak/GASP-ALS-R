...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GarUtility)

FString UGarUtility::NameToDisplayString(const FName& Name, const bool bNameIsBool)
{
	return FName::NameToDisplayString(Name.ToString(), bNameIsBool);
}

float UGarUtility::GetAnimationCurveValueFromCharacter(const ACharacter* Character, const FName& CurveName)
{
...
FGameplayTagContainer UGarUtility::GetChildTags(const FGameplayTag& Tag)
{
	return UGameplayTagsManager::Get().RequestGameplayTagChildren(Tag);
}

FName UGarUtility::GetSimpleTagName(const FGameplayTag& Tag)
{
...
float UGarUtility::GetFirstPlayerPingSeconds(const UObject* WorldContext)
{
...
FTransform UGarUtility::ExtractRootTransformFromMontage(const UAnimMontage* Montage, const float Time)
{
	// Based on UMotionWarpingUtilities::ExtractRootTransformFromAnimation().

	if (!ensure(IsValid(Montage)) || !ensure(Montage->SlotAnimTracks.Num() > 0))
	{
...
FTransform UGarUtility::ExtractLastRootTransformFromMontage(const UAnimMontage* Montage)
{
	// Based on UMotionWarpingUtilities::ExtractRootTransformFromAnimation().

	if (!ensure(IsValid(Montage)) || !ensure(Montage->SlotAnimTracks.Num() > 0) ||
		!ensure(Montage->SlotAnimTracks[0].AnimTrack.AnimSegments.Num() > 0))
	{
		return FTransform::Identity;
	}
...
bool UGarUtility::ShouldDisplayDebugForActor(const AActor* Actor, const FName& DisplayName)
{
...
void UGarUtility::DrawHalfCircle(const UObject* WorldContext, const FVector& Location, const FVector& XAxis,
...
void UGarUtility::DrawQuarterCircle(const UObject* WorldContext, const FVector& Location, const FVector& XAxis,
									const FVector& YAxis, const float Radius, const FLinearColor& Color,
									const float Duration, const float Thickness, const uint8 DepthPriority)
{
#if ENABLE_DRAW_DEBUG
...
void UGarUtility::DrawDebugSphereAlternative(const UObject* WorldContext, const FVector& Location, const FRotator& Rotation,
...
void UGarUtility::DrawDebugLineTraceSingle(const UObject* WorldContext, const FVector& Start, const FVector& End, const bool bHit,
...
void UGarUtility::DrawDebugSweepSphere(const UObject* WorldContext, const FVector& Start, const FVector& End, const float Radius,
...
void UGarUtility::DrawDebugSweepSingleSphere(const UObject* WorldContext, const FVector& Start, const FVector& End,
...
void UGarUtility::DrawDebugSweepSingleCapsule(const UObject* WorldContext, const FVector& Start, const FVector& End,
...
void UGarUtility::DrawDebugSweepSingleCapsuleAlternative(const UObject* WorldContext, const FVector& Start, const FVector& End,
