#include "Nodes/GarRigUnits.h"

#include "Utility/GarMath.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(GarRigUnits)

FGarRigVMFunction_Clamp01Float_Execute()
{
	Result = UGarMath::Clamp01(Value);
}

void FGarRigVMFunction_ExponentialDecayVector::Initialize()
{
	bInitialized = false;
}

FGarRigVMFunction_ExponentialDecayVector_Execute()
{
	...
	Current = UGarMath::ExponentialDecay(Current, Target, ExecuteContext.GetDeltaTime(), Lambda);
}

void FGarRigUnit_CalculatePoleVector::Initialize()
{
	bInitialized = false;
}

FGarRigUnit_CalculatePoleVector_Execute()
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_RIGUNIT()
	...
	if (!UGarMath::TryCalculatePoleVector(Hierarchy->GetGlobalTransformByIndex(CachedItemA, bInitial).GetLocation(), NewItemBLocation,
...
void FGarRigUnit_HandIkRetargeting::Initialize()
{
	bInitialized = false;
}

FGarRigUnit_HandIkRetargeting_Execute()
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_RIGUNIT()
