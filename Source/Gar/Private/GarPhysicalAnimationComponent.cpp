...
void FGarRagdollingState::Start(UGarRagdollingSettings* NewSettings, const UGarPhysicalAnimationComponent* PhysicalAnimation)
{
	...
	// 必要なチャンネルに Block を設定
	Character->GetCapsuleComponent()->SetCollisionResponseToChannel(ECC_WorldStatic, ECR_Block);     // 地形
	Character->GetCapsuleComponent()->SetCollisionResponseToChannel(ECC_WorldDynamic, ECR_Block);    // 動的オブジェクト
	Character->GetCapsuleComponent()->SetCollisionResponseToChannel(ECC_Pawn, ECR_Block);            // 他のPawn
	Character->GetCapsuleComponent()->SetCollisionResponseToChannel(ECC_Visibility, ECR_Block);      // トレース用
	Character->GetCapsuleComponent()->SetCollisionResponseToChannel(ECC_Camera, ECR_Ignore);         // カメラは無視（任意）
...
void FGarRagdollingState::End(const UGarPhysicalAnimationComponent* PhysicalAnimation)
{
	...
	Character->GetCapsuleComponent()->SetCollisionProfileName(UCollisionProfile::Pawn_ProfileName);
